      ...
      <section className="py-[50px] px-8 bg-slate-50">
        <div className="max-w-content mx-auto">
          <div className="flex flex-wrap gap-3">
            ...
            {related.map((e) => (
              <Link key={e.slug} href={`/kuendigung/${e.slug}/`} className="py-2.5 px-5 rounded-full border border-border text-[0.85rem] font-semibold text-ink no-underline hover:border-gold hover:text-gold transition-all bg-white">
                {e.h1.replace(/ — .*$/, '')}
              </Link>
            ))}
          </div>
